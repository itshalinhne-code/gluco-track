...
import 'package:doctorapp/model/clinic_model.dart';
import 'package:doctorapp/model/prescription_model.dart';
import 'package:doctorapp/model/prescription_pre_field_model.dart';
import 'package:doctorapp/utilities/colors_constant.dart';
import 'package:doctorapp/widget/loading_Indicator_widget.dart';
  ...
  Future<void> uploadPdfAndJson() async {
    ...
    try {
      _pageImages[_currentPageIndex] =
          (await _drawingControllers[_currentPageIndex].getImageData())?.buffer.asUint8List();
...
// Collect JSON data from all pages
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['response'] == 201) {
          if (jsonData['status'] == "error") {
            IToastMsg.showMessage("Có lỗi xảy ra");
          } else {
          ...
          IToastMsg.showMessage("Đã tải lên thành công");
          final file = jsonData['file'];
          launchUrl(Uri.parse("${ApiContents.imageUrl}/$file"));
        }
      } else {
        debugPrint("Tải lên thất bại: ${response.statusCode}");
      }
    } catch (e) {
      debugPrint("⚠️ Lỗi khi tải lên dữ liệu: $e");
    }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: uploadFileLoading || _isLoading
            ? null
            : AppBar(
                leading: PopupMenuButton<Color>(
                  icon: const Icon(Icons.color_lens),
                  onSelected: (Color value) => _drawingControllers[_currentPageIndex].setStyle(
                            ...
                            return Slider(
                              ...
                              onChanged: (double v) {
                                ...
                                _drawingControllers[_currentPageIndex].setStyle(
                                  color: _drawingControllers[_currentPageIndex]
                                      .drawConfig
                                      .value
                                      .color
                ...
                title: const Text('Viết đơn thuốc'),
        ...
        body: uploadFileLoading || _isLoading
                        ...
                        builder: (BuildContext context, BoxConstraints constraints) {
                          return Container(
                            ...
                            child: DrawingBoard(
                              ...
                              background: !_isLoadingImg
                                  ? Container(
                                      color: Colors.white,
                                      child: CustomPaint(
                                        size: Size(595.28, 841.89),
                                        painter: PreFilledTextPainter(logoImage,
                                            prescriptionPreFieldModel), // Paints the pre-filled text
                                      ),
                                    )
                                  : Container(),
                              ...
                              defaultToolsBuilder: (Type t, _) {
                                List<DefToolItem> tools = DrawingBoard.defaultTools(
                                  t,
                                  _drawingControllers[_currentPageIndex],
                                );
                                ...
                                DefToolItem? eraserTool = tools.firstWhere(
                                  ...
                                  orElse: () => DefToolItem(
                                    // Provide a default Eraser tool if not found
                                    icon: CupertinoIcons.bandage,
                                    isActive: t == Eraser,
                                    onTap: () => _drawingControllers[_currentPageIndex]
                                        ...
                                        try {
                                          ...
                                          if (imageFile != null && context.mounted) {
                                            ...
                                            _drawingControllers[_currentPageIndex].setPaintContent(
                                              ImageContent(image,
                                                  imageUrl:
                                                      imageFile.path), // Pass correct Image type
                                            );
                                          }
                                        } catch (e) {
                                          if (context.mounted) {
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Text("Trang ${_currentPageIndex + 1} / ${_drawingControllers.length}"),
                          IconButton(
  ...
  void _changePage(int index) async {
    ...
    debugPrint(
        "Stored Image Data for Page $_currentPageIndex: ${_pageImages[_currentPageIndex] != null}");
    setState(() {
      _isLoading = false;
    });
  }
  ...
  void getAndSetData() async {
    ...
    _pageImages.add(newPageImage);
    clinicModel =
        await ClinicService.getDataById(clinicId: widget.prescriptionPreFieldModel?.clinicId);
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("Bạn có chắc chắn muốn xóa trang hiện tại (${_currentPageIndex + 1})",
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("Bạn có chắc chắn muốn lưu và tải lên các trang vào đơn thuốc của bệnh nhân?",
                  style: const TextStyle(fontWeight: FontWeight.w400, fontSize: 14)),
              const SizedBox(height: 10),
            ],
          ),
  ...
  void _deleteCurrentPage() {
    if (_drawingControllers.length > 1) {
      ...
      debugPrint("Đã xóa trang. Số trang còn lại: ${_drawingControllers.length}");
    } else {
      debugPrint("Không thể xóa trang cuối cùng.");
    }
  }
...
extension PaintContentExtension on PaintContent {
  ...
  bool isNear(Offset point) {
    if (this is ImageContent) {
      final imageContent = this as ImageContent;
      final rect =
          Rect.fromPoints(imageContent.startPoint, imageContent.startPoint + imageContent.size);
