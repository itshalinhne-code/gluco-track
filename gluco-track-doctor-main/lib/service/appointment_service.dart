...
class AppointmentService {
  ...
  static List<AppointmentModel> dataFromJson(jsonDecodedData) {
    return List<AppointmentModel>.from(
        jsonDecodedData.map((item) => AppointmentModel.fromJson(item)));
  }

  static Future<List<AppointmentModel>?> getData(
      {required int start, required int end, String search = ""}) async {
  ...
  static Future updateStatus({
    ...
    Map body = {'status': status, 'id': appointmentId};
    final res = await PostService.postReq(updateAppointmentStatusUrl, body);
    return res;
  }

  static Future updateStatusToReject({required String appointmentId}) async {
    Map body = {'appointment_id': appointmentId};
    final res = await PostService.postReq(appointmentRejectUrl, body);
    return res;
  }

  static Future updateStatusToCancel({required String appointmentId}) async {
    Map body = {'appointment_id': appointmentId, "status": "Approved"};
  ...
  static Future updateStatusToResch({
    ...
    Map body = {'date': date, 'id': appointmentId, "time_slots": timeSlots};
