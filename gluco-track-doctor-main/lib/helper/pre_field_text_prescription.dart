...
class PreFilledTextPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawKeyValueText(
        String label, String value, Offset position, TextStyle style, Canvas canvas,
    ...
    yPos += 30;
    drawText(
        prescriptionPreFieldModel?.clinicAddress ?? "", Offset(xPosLeft, yPos), smallTextStyle);
    yPos += 20;
    drawText(
        'Phone: ${prescriptionPreFieldModel?.phone ?? ""}', Offset(xPosLeft, yPos), smallTextStyle);
    yPos += 20;
    drawText(
        'Email: ${prescriptionPreFieldModel?.email ?? ""}', Offset(xPosLeft, yPos), smallTextStyle);
    yPos += 20;
    ...
    drawKeyValueText("Patient ID", prescriptionPreFieldModel?.patientId ?? "",
        Offset(xPosLeft, yPos), textStyle, canvas);
    yPos += 20;
    drawKeyValueText("Appointment ID", prescriptionPreFieldModel?.appointmentID ?? "",
        Offset(xPosLeft, yPos), textStyle, canvas);
    yPos += 20;
    drawKeyValueText("Name", prescriptionPreFieldModel?.patientName ?? "", Offset(xPosLeft, yPos),
        textStyle, canvas);
    yPos += 20;
    drawKeyValueText("Age", prescriptionPreFieldModel?.patientAge ?? "", Offset(xPosLeft, yPos),
        textStyle, canvas);
    yPos += 20;
    drawKeyValueText("Gender", prescriptionPreFieldModel?.patientGender ?? "",
        Offset(xPosLeft, yPos), textStyle, canvas);
    yPos += 20;
    drawKeyValueText("Phone", prescriptionPreFieldModel?.patientPhone ?? "", Offset(xPosLeft, yPos),
        textStyle, canvas);
    yPos += 15;

    // Doctor Details (Right Side, Aligned to Right)
    drawText(prescriptionPreFieldModel?.doctorName ?? "", Offset(size.width, yPos - 75), textStyle,
        alignRight: true);
    yPos += 10;
    drawText(prescriptionPreFieldModel?.doctorSpec ?? "", Offset(size.width, yPos - 60), textStyle,
        alignRight: true);
    yPos += 10;
    drawText(prescriptionPreFieldModel?.doctorDept ?? "", Offset(size.width, yPos - 45), textStyle,
        alignRight: true);
    // yPos += 10;
